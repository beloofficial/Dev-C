...
Object.extendObject(
	...
	build: {
		comment: function(id, text){
			...
			this._highlight(entry_reply_item);
	    ...
	    _highlight: function(node) {
	        ...
	        for (var i = 0; i < pres.length; i++) {
              var code = pres[i].getElementsByTagName('code');
              if (code.length) hljs.highlightBlock(code[0], hljs.tabReplace);
            }
	    }
	},
	
	_addComments: function(xmlNodes, mess_box){
		var replies = [];
		...
		for(var i=0; i<xmlNodes.comment.length; i++) {
			...
			if(xmlNodes.comment[i].getAttribute('parent_id')!=0) {
				...
				if (!mess_box){
					...
					entry_reply_item.getElement('p.reply').dispose(true);
					$(reply_holder).grab(entry_reply_item, 'top')
				}
	...
	xmlResponseHandler_comment: function(xmlNodes, params){
		if(xmlNodes.message && xmlNodes.message.length && xmlNodes.comment && xmlNodes.comment.length) {
			...
			if (!jsfldhld.className.match(/message-box-form/)){
				jsfldhld.addClass('hidden');
				this._addComments(xmlNodes);
			} else{
				this._addComments(xmlNodes, true);
				
				params.form['comment[message]'].value="";
			}
		...
		$(params.form).removeClass('loading');
	...
	xmlResponseHandler_preview: function(xmlNodes, params){
		if(xmlNodes.preview && xmlNodes.preview.length) {
			...
			$(params.form).removeClass(params.loadingClassName);
			$(params.form).removeClass('loading');
			if (params.form.input_preview){
				$(params.form.input_preview).removeClass('loading');
			}
			this.build._highlight(previewHolder);
		}
	},
	...
	sendComment: function(form, submitType, node) {
	...
	sendOnEnter: function(textarea) {
		textarea.onkeyup = function(ev) {
			...
			if(key==13 && ev.ctrlKey) {
				commentForm.sendComment(this.form, 'comment', this.form);
			}
		}
	},
...
tm.init.add(function(){
	...
	if (c_form_node) {
		...
		if (c_form_node.input_preview){
			
			c_form_node.input_preview.addEvent('click',function(){
				commentForm.sendComment(c_form_node, 'preview', c_form_node.input_preview)
			});
		}
		...
		var c_form_submit = function(){
			if (c_form_node.last_text == c_form_node['comment[message]'].value){
				...
				commentForm.sendComment(c_form_node, comment_type, c_form_node); 
		...
		/** 
		 * галочка "подписаться на новые комментарии".
		 */
    var subscribe_checkbox = $('subscribe_comments');
    if(subscribe_checkbox){
  		subscribe_checkbox.addEvent('click', function(e){		  
  		  eval('var data = '+subscribe_checkbox.getProperty('rel')+';');
  		  data.action = subscribe_checkbox.get('checked') ? 'subscribe' : 'unsubscribe';
        var req = new Request.JSON({
            url: '/json/subscription/',
            data: data,
            method: 'post',
            onSuccess: function(json){
              futu_alert(FAT.comments_header, 'Настройки уведомления о новых комментариях успешно сохранены', true, 'save'); 
            }
          }).send();
      });
    }
