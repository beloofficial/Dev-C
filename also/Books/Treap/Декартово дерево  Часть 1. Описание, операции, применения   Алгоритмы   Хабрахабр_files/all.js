	...
	favoritesSendOnload : function(ajaxObj, params) {
		if(ajaxObj && ajaxObj.responseXML){
			...
			if(xmlNodes.result && xmlNodes.result.length) {
				...
				if (params.target_type == 'posts' || params.target_type == 'questions') {
					$(params.button).getParent('.js-to_favs_holder').switchClass('fav_added', 'to-favs');

					var div = params.button.getParent('div.hentry');
                    var count = div.getElement('.favs_count');
                    var prev = parseInt(count.get('text')) || 0;
					if (previous_off) {
            count.set('text', prev + 1);
						params.button.title ='Удалить из избранного';
    				if (params.target_type == 'questions') {
	...
	temp.favourites_add = {
		...
		'setTags': function (tags, unfav) {
			...
			if (tags_holder) {
				...
				if (unfav) {
				    if (edit_tags) edit_tags.dispose();
				} else {
					if (!edit_tags) {
...
var FAT = {
	...
	// профиль
	profile_header : 'Хабрахабр - Настройки',
	...
	failture : 'Хабрахабр — Ошибка',
	// ajax error
	ajax_error : 'Хабрахабр — Ошибка Ajax-запроса'
};
		...
		if (params.get_type == 'regions') {
			...
			if (xmlNodes.region && xmlNodes.region.length) {
				...
				for (var i = 0; i < l; i++) {
					...
					$(selectRegions).removeClass('hidden');
			...
			if (xmlNodes.city && xmlNodes.city.length) {
				...
				for (var i = 0; i < l; i++) {
					...
					$(selectCities).removeClass('hidden');
...
blogsHandler = {
	...
	xml: {
		responseNodes: {
			error : 'error',
			members_count_str : 'members_count_str',
			result : 'message',
			param : 'param',
		...
		if(ajaxObj && ajaxObj.responseXML){
			...
			if(xmlNodes.result && xmlNodes.result.length) {
				if(xmlNodes.param && xmlNodes.param.length) {
					...
					$('members_count').set('html', xmlNodes.members_count_str[0].firstChild.data);
				}
			}
		}
	},
