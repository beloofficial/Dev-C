		...
		if (infopanel) {
			...
			var onComplete = function(responseHTML, responseXML){
				...
				if (errors.length) {
					...
					$('usercharge').innerHTML = responseXML.getElementsByTagName('charge_string')[0].firstChild.data
					scoreNode.set("text",score);
					hasVoteMarkNode.addClass("voted").removeClass("voted_minus").removeClass("voted_plus").removeClass("positive").removeClass("negative");
			...
			commentsContainer.addEvent("click",function(e){
				...
				//Changed at 20/07/2010
				if (nodeClassName.indexOf("vote-for-answer") != -1) {
					//Answers voting
					var commentId = e.target.getAttribute("rev").replace("voter-for-answer:",""), voteType;
